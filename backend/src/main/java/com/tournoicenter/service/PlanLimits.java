...
public record PlanLimits(int maxTournaments, int maxTeams, boolean realtime, boolean customRules,
                          boolean sponsorBanner, int maxConcurrent) {

    public static final Map<Plan, PlanLimits> BY_PLAN = Map.of(
            Plan.FREE, new PlanLimits(1, 14, false, false, false, 50),
            Plan.CLASSIC, new PlanLimits(Integer.MAX_VALUE, Integer.MAX_VALUE, true, false, false, 500),
            Plan.PRO, new PlanLimits(Integer.MAX_VALUE, Integer.MAX_VALUE, true, true, true, 3000)
    );
